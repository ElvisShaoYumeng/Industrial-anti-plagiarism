...
Case1 = ['K001', 'KA01', 'KA06', 'KI01', 'KI07']  # 五分类
label1 = [i for i in range(5)]

#working condition
WC = {0:"N15_M07_F10",1:"N09_M07_F10",2:"N15_M01_F10",3:"N15_M07_F04"}
...
def get_files(root, condition, label, nec):
    data = []
    lab = []
    state = WC[condition]  # WC[0] can be changed to different working states
    for j in range(len(Case1)):
        state1 = Case1[j]
        name3 = state + "_" + state1 + "_" + str(1)
        path3 = os.path.join(root, state1, name3 + ".mat")
        data3, lab3 = data_load(path3, name=name3, label=label)
        data += data3
        lab += lab3
    return [data,lab]
...
def data_load(filename,name,label):
    ...
    return data, lab
    # while  n<nec:
    #     name1 = name + "_" + str(n + 1)
    #     filename1 = os.path.join(root, state1, name1 + ".mat")
    #     fl = loadmat(filename1)[name1]
    #     fl = fl[0][0][2][0][6][2]  # Take out the data
    #     fl = fl.reshape(-1, 1)
    #     data.append(fl[start:end])
    #     lab.append(label)
...
class Dataset_PU(object):
    ...
    def __init__(self, args, condition, mode, location):
        ...
        if not os.path.exists('./Data/PU/generated/x_c0.npy'):
            ...
            np.save('./Data/generated/x_c0.npy', x_c0);np.save('./Data/generated/y_c0.npy', y_c0)
            x_c1, y_c1 = self.gene_data_of_given_condition( 1);print(x_c1.shape, y_c1.shape)
            np.save('./Data/generated/x_c1.npy', x_c1);np.save('./Data/generated/y_c1.npy', y_c1)
            x_c2, y_c2 = self.gene_data_of_given_condition( 2);print(x_c2.shape, y_c2.shape)
            np.save('./Data/generated/x_c2.npy', x_c2);np.save('./Data/generated/y_c2.npy', y_c2)
            x_c3, y_c3 = self.gene_data_of_given_condition( 3);print(x_c3.shape, y_c3.shape)
            np.save('./Data/generated/x_c3.npy', x_c3);np.save('./Data/generated/y_c3.npy', y_c3)
    ...
    def gene_data_of_given_condition(self, condition):                     #为某种工况产生数据并保存
        ...
        x = np.zeros((self.args.ways, self.args.signal_length,1));y = np.zeros((self.args.ways))
    ...
    def __getdata__(self, condition, mode, location):
        ...
        x = np.load('./Data/generated/x_c'+str(condition)+'.npy')
        y = np.load('./Data/generated/y_c'+str(condition)+'.npy')
...
argparser.add_argument('--ways', type=int, help='n way', default=5)
